...
import { NEARBY_RESTAURANTS } from '../data/placesPoints';
import { MOCK_USER_PROFILE, searchFilter, evaluateRestoranTap, getRestoranBannerConfig } from '../data/placesScenarios';
...
export const YakindakiRestoranlarScreen: React.FC = () => {
  ...
  const banner = getRestoranBannerConfig(MOCK_USER_PROFILE);

  const filtered = useMemo(() => {
    let list = NEARBY_RESTAURANTS.filter((b) => searchFilter(search, b.name));
  ...
  const handlePlaceTap = (placeId: string) => {
    ...
    switch (action.type) {
      ...
      case 'modal-no-balance':
        setInfoModal({ visible: true, message: 'Restoranlarda harcayabilecegin bakiyen bulunmamaktadir.' });
        break;
      case 'modal-transfer-required':
        setAktarimModal({ visible: true, message: "Restoranlarda harcama yapabilmen icin Pluxee Yemek'e aktarim yapman gerekmektedir.", target: 'yemek' });
        break;
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text variant="title.mobileCard" color="inverse" align="center" style={styles.title}>Yakindaki Restoranlar</Text>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.toolbar}>
          <TouchableOpacity style={styles.filtreChip} onPress={() => nav.navigate('PlacesFilter', { context: 'restoran', currentFilters: filters, onApply: setFilters })}>
...
export default YakindakiRestoranlarScreen;
